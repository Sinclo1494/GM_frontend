...
import TypeMarquePage from "../../pages/TypeMarquePage";
...
describe("TypeMarquePage", () => {
  beforeEach(() => {
    resetAllMocks();
  });

  it("renders without crashing", () => {
    render(<TypeMarquePage />);
    expect(screen.getByTestId("crud-page")).toBeTruthy();
  });

  it("passes the correct title to CrudPage", () => {
    render(<TypeMarquePage />);
    expect(screen.getByTestId("crud-title").textContent).toBe("Types de Marque");
  });

  it("passes the correct endpoint to CrudPage", () => {
    render(<TypeMarquePage />);
    expect(screen.getByTestId("crud-endpoint").textContent).toBe("type-marque");
  });
