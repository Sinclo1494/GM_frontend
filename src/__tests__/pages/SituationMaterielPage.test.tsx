...
import SituationMaterielPage from "../../pages/SituationMaterielPage";
...
describe("SituationMaterielPage", () => {
  beforeEach(() => {
    resetAllMocks();
  });

  it("renders without crashing", () => {
    render(<SituationMaterielPage />);
    expect(screen.getByTestId("crud-page")).toBeTruthy();
  });

  it("passes the correct title to CrudPage", () => {
    render(<SituationMaterielPage />);
    expect(screen.getByTestId("crud-title").textContent).toBe("Situations Matériel");
  });

  it("passes the correct endpoint to CrudPage", () => {
    render(<SituationMaterielPage />);
    expect(screen.getByTestId("crud-endpoint").textContent).toBe("situation-materiel");
  });
