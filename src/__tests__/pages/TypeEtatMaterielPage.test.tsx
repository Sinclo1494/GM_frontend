...
import TypeEtatMaterielPage from "../../pages/TypeEtatMaterielPage";
...
describe("TypeEtatMaterielPage", () => {
  beforeEach(() => {
    resetAllMocks();
  });

  it("renders without crashing", () => {
    render(<TypeEtatMaterielPage />);
    expect(screen.getByTestId("crud-page")).toBeTruthy();
  });

  it("passes the correct title to CrudPage", () => {
    render(<TypeEtatMaterielPage />);
    expect(screen.getByTestId("crud-title").textContent).toBe("Types d'État Matériel");
  });

  it("passes the correct endpoint to CrudPage", () => {
    render(<TypeEtatMaterielPage />);
    expect(screen.getByTestId("crud-endpoint").textContent).toBe("type-etat-materiel");
  });
