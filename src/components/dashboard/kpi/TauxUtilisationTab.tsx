...
import type { DashboardData, DashboardFilters, TauxUtilisationEvolutionPoint, TauxUtilisationBreakdownItem } from "../../../types/dashboard";
...
interface TauxUtilisationTabProps {
  data: DashboardData;
...
const TauxUtilisationTab = ({ data, filters }: TauxUtilisationTabProps) => {
  const evolution = useMemo<TauxUtilisationEvolutionPoint[]>(() => {
    return data.tauxUtilisationEvolution ?? [];
  }, [data.tauxUtilisationEvolution]);

  const breakdown = useMemo<TauxUtilisationBreakdownItem[]>(() => {
    return data.tauxUtilisationBreakdown ?? [];
  }, [data.tauxUtilisationBreakdown]);
  ...
  const tauxUtilisation = useMemo<number>(() => {
    if (totalPotentiel > 0) {
      return (totalHeuresService / totalPotentiel) * 100;
    }
    return 0;
  }, [totalHeuresService, totalPotentiel]);
    ...
    datasets: [
      {
        ...
        data: evolution.map((p) =>
          p.potentiel > 0 ? (p.heures_service / p.potentiel) * 100 : null
        ),
        borderColor: "#22c55e",
      ...
      {
        label: "Seuil alerte (70%)",
        data: evolution.map(() => 70),
        borderColor: "#ef4444",
        borderDash: [5, 5],
        pointRadius: 0,
        fill: false,
  ...
  const status = tauxUtilisation >= 70 ? "success" : "danger";
  const statusColor =
    tauxUtilisation >= 70 ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400";
  const thresholdLabel = tauxUtilisation >= 70 ? "Atteint" : "Alerte";
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <div className={`${components.card} p-4 relative overflow-hidden`}>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-dark-text-secondary">
            ...
            {formatNumber(totalHeuresService, 1)} H
          </p>
        </div>
      ...
      <div className={components.card}>
        <h2 className={components.sectionTitle}>
          Évolution mensuelle du taux d'utilisation
        </h2>
        {evolution.length === 0 ? (
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {paginatedBreakdown.map((row, idx) => {
                    const itemStatus = row.taux_utilisation >= 70 ? "success" : "danger";
                    return (
                      <tr
                        key={row.code}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          ...
                          >
                            {row.taux_utilisation >= 70 ? "Atteint" : "Alerte"}
                          </span>
                        </td>
                      </tr>
                    );
...
export default TauxUtilisationTab;
