...
const PanneTab = ({ data, filters }: PanneTabProps) => {
  const evolution = useMemo<PanneEvolutionPoint[]>(() => {
    return data.tauxPanneEvolution ?? [];
  }, [data.tauxPanneEvolution]);
  ...
  const tauxPanne = useMemo<number>(() => {
    if (totalPotentiel > 0) {
      return (totalPanne / totalPotentiel) * 100;
    }
    return 0;
    ...
    datasets: [
      {
        ...
        data: evolution.map((p) =>
          p.potentiel > 0 ? (p.heures_panne / p.potentiel) * 100 : 0
        ),
        borderColor: "#ef4444",
  ...
  const status = tauxPanne > 10 ? "danger" : "success";
  const statusColor =
    tauxPanne > 10 ? "text-red-700 dark:text-red-400" : "text-green-700 dark:text-green-400";
  const thresholdLabel = tauxPanne > 10 ? "Alerte" : "Normal";
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <div className={`${components.card} p-4 relative overflow-hidden`}>
          ...
          <p className={`mt-2 text-3xl font-bold ${statusColor}`}>
            {totalPotentiel > 0 ? `${tauxPanne.toFixed(1)}%` : "—"}
          </p>
      ...
      <div className={components.card}>
        <h2 className={components.sectionTitle}>
          Évolution mensuelle du taux de panne
        </h2>
        {evolution.length === 0 ? (
